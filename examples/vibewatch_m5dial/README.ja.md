...
UIはメインのVibe Watchファームウェア（`src/main.cpp`）の見た目に合わせています。
太いアクセントキーラインを持つフラットな塗りの円、単なる略称テキストの代わりに
ベクターアイコン（マイク、雷、チェック、バツ、プランのピル型トグル、アシスタント
の顔）、選択中のAgentに表示される2トーンのセレクションハロー、ラベルとステータス
表示に使われるOrbitronフォント、そしてAgentレイヤーをプレビューする拡大するリング
と6つの周回ドットによるブートアニメーションです。
